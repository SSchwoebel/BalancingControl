...
def plot_beliefs(fname, plot_context=True, plot_policies=True, plot_actions=True, plot_prior_pol=True, fit_params=None, save_num=-1, prefix=""):
    ...
    for k in range(1,w.agent.nh):
        plt.plot(times, Rho[:,k,k], label="lever "+str(k), c=arm_cols[k-1], linewidth=4)
    plt.ylim([-0.01,1.01])
    plt.xlim([0,times[-1]])
    ...
    plt.savefig(os.path.join(folder,prefix+"_env.svg"), bbox_extra_artists=(lgd,), bbox_inches='tight')
    plt.savefig(os.path.join(folder,prefix+"_env.png"), bbox_extra_artists=(lgd,), bbox_inches='tight')
    ...
    for i in range(repetitions):
        ...
        if plot_policies:
            ...
            post_pol = (w.agent.posterior_policies[:,0,:]* w.agent.posterior_context[:,0,None]).sum(axis=2)
            plt.figure(figsize=(10,5))
            for k in range(1,w.agent.nh):
                plt.plot(times, Rho[:,k,k], label="lever "+str(k), c=arm_cols[k-1], linewidth=4, alpha=0.5)
            plt.plot(times, post_pol[:,1], ".", label="action "+str(1+1), color=action_cols[0], ms=10)
            if fit_params is not None:
            ...
            #if save_num == i:
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_posterior_actions.svg"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_posterior_actions.png"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.show()

            plt.figure(figsize=(10,5))
            plt.plot(np.argmax(post_pol, axis=1)+1, ".", label="action", color=action_cols[0], ms=10)
            if fit_params is not None:
            ...
            plt.xticks(fontsize=18)
            plt.xlabel("trial", fontsize=20)
            plt.ylabel("inferred action", fontsize=20)
            ax = plt.gca()#.twinx()
            ax.set_ylim([0.9,3.1])
            ax.set_yticks([1,2,3])
            ax.set_yticklabels(["$a_{1}$","$a_{2}$", "$a_{3}$"],fontsize=18)
            #if save_num == i:
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_action.svg"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_action.png"), bbox_extra_artists=(lgd,), bbox_inches='tight')
...
#            plt.show()
        ...
        if plot_context:
            plt.figure(figsize=(10,5))
            for k in range(1,w.agent.nh):
                plt.plot(w.environment.Rho[:,k,k], label="lever "+str(k), c=arm_cols[k-1], linewidth=4, alpha=0.5)
            plt.plot(w.agent.posterior_context[:,1,1], ".", label="context "+str(1+1), color=context_cols[0], ms=10)
            if fit_params is not None:
                print(results_c_param[i])
                fct = fit_functions[results_c_param_type[i]]
                param = results_c_param[i,results_c_param_type[i]:]
            ...
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_posterior_context.png"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.show()


            plt.figure(figsize=(10,5))
            plt.plot(np.argmax(w.agent.posterior_context[:,1,:],axis=1)+1, ".", label="context", color=context_cols[0], ms=10)
            if fit_params is not None:
                print(results_c_param[i])
                fct = fit_functions[results_c_param_type[i]]
                param = results_c_param[i,results_c_param_type[i]:]
            ...
            plt.xticks(fontsize=18)
            plt.xlabel("trial", fontsize=20)
            plt.ylabel("inferred context", fontsize=20)
            ax = plt.gca()#.twinx()
            ax.set_ylim([0.9,3.1])
            ax.set_yticks([1,2,3])
            ax.set_yticklabels(["$c_{1}$","$c_{2}$", "$c_{3}$"],fontsize=18)
            #if save_num == i:
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_context.svg"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.savefig(os.path.join(fname[:-5]+"_run"+str(i)+"_context.png"), bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.show()
...
def plot_run(check=False, naive=False, deval=False, context=None):
    ...
    plot_beliefs(fname, plot_context=True, plot_policies=True, plot_actions=False, plot_prior_pol=True, fit_params=fit_params, prefix=test)
