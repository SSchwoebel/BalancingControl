...
class Group2ContextPerception(object):
    ...
    def update_messages(self, tau, t, possible_policies):
        ...
        fwd = [ar.zeros((self.nh, self.npi, self.nc, self.npart, self.nsubs)).to(device)+self.prior_states[:,None,None,None]]
    ...
    def update_beliefs(self, tau, t, observation, reward, prev_response, possible_policies, context_obs=None):
        ...
        if self.infer_context:
            if tau>0 and t==0:
                self.miniblock_context_prior.append(ar.einsum('cz...,z...->c...', 
    ...
    def update_beliefs_policies(self, tau, t):
        ...
        self.posterior_policies.append(ar.einsum('pc...,c...->p...', posterior_policies, self.posterior_contexts))
    ...
    def update_beliefs_context(self, tau, t, context_obs=None):
        ...
        if self.nc == 1:
            ...
            if t == self.T-1:
                ...
                one_hot_pols = ar.nn.functional.one_hot(chosen_pol, num_classes=self.npi).permute(2,0,1).float()
...
#        self.dirichlet_pol_params[chosen_pol,:] += posterior_context.sum(axis=0)/posterior_context.sum()
        ...
        pol_update = chosen_pol[:,None,:,:]*self.mask[tau][None,None,:,:]*self.posterior_context[-1][None,:,:,:]
