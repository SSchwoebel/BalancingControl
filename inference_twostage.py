...
class GroupInference(object):
    ...
    def model(self):
        npar = 3
        mu = pyro.param('mu', ar.ones(2*npar)).to(device)
        sig = pyro.param('sig', ar.ones(2*npar), constraint=ar.distributions.constraints.positive).to(device)

        locs = pyro.sample('locs', dist.Normal(mu, sig).to_event(1)).to(device)

        alpha_lamb_pi = ar.exp(locs[...,0])
        beta_lamb_pi = ar.exp(locs[...,1])

        alpha_lamb_r = ar.exp(locs[...,2])
        beta_lamb_r = ar.exp(locs[...,3])

        conc_dec_temp = locs[...,4]
        rate_dec_temp = ar.exp(locs[...,5])
        ...
        with pyro.plate('subject', self.nsubs) as ind:
            ...
            lamb_pi = pyro.sample('lamb_pi', dist.Beta(alpha_lamb_pi, beta_lamb_pi)).to(device)
            lamb_r = pyro.sample('lamb_r', dist.Beta(alpha_lamb_r, beta_lamb_r)).to(device)
            dec_temp = pyro.sample('dec_temp', dist.LogNormal(conc_dec_temp, rate_dec_temp)).to(device)
    ...
    def guide(self):
        ...
        scale_tril = pyro.param("scale_tril", ar.tril(ar.eye(2*npar)), constraint=ar.distributions.constraints.lower_cholesky).to(device)
        mu = pyro.param("mu", ar.zeros(2*npar), constraint=ar.distributions.constraints.real_vector).to(device)
        locs = pyro.sample('locs', dist.MultivariateNormal(mu, scale_tril)).to(device)
