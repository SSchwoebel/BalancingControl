...
class Group2Inference(object):
    ...
    def model(self):
        ...
        with pyro.plate('subject', self.nsubs) as ind:
            ...
            param_dict = {"pol_lambda": ar.sigmoid(locs[...,0]),
                          "r_lambda": ar.sigmoid(locs[...,1]), "dec_temp": 10*ar.sigmoid(locs[...,2]),}
                          #"h": ar.sigmoid(locs[...,3])}
            # print(param_dict)
    ...
    def sample_posterior(self, n_samples=5):
        ...
        for i in range(n_samples):
            ...
            for key in sample.keys():
                sample.setdefault(key, ar.ones(1))
            lamb_pi = sample["pol_lambda"]
            lamb_r = sample["r_lambda"]
            #alpha_0 = sample["h"]
            dec_temp = sample["dec_temp"]
...
class GroupAlphaInference(object):
    ...
    def model(self):
        ...
        with pyro.plate('subject', self.nsubs) as ind:
            ...
            pol_lambda = pyro.sample("pol_lambda", dist.Beta(alpha_lamb_pi, beta_lamb_pi))
            r_lambda = pyro.sample("r_lambda", dist.Beta(alpha_lamb_r, beta_lamb_r))

            param_dict = {"pol_lambda": pol_lambda,
                "r_lambda": r_lambda, "dec_temp": ar.exp(locs[...,4]), }
    ...
    def guide(self):
        ...
        return {'tau': tau, 'mu': mu, 'locs': locs, "pol_lambda": pol_lambda,
                "r_lambda": r_lambda, "dec_temp": ar.exp(locs[...,4]), }
                #"h": ar.sigmoid(locs[...,3])}
...
class GroupHInference(object):
    ...
    def guide(self):
        ...
        return {'tau': tau, 'mu': mu, 'locs': locs, "pol_lambda": ar.sigmoid(locs[...,0]),
                "r_lambda": ar.sigmoid(locs[...,1]), "dec_temp": 10*ar.sigmoid(locs[...,2]),
                "alpha_0": ar.exp(locs[...,3])+1, "h": 1./(ar.exp(locs[...,3])+1)}
...
class GroupMbmfInference(object):
